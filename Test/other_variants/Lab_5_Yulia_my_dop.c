...
int main (int argc, char *argv[]) {
  ...
  for (int i = 0; i < len; i++) { // проходим по всей строке
  ...
  printf("\nEnter first word:\n");
  char* word = readstr(); // считываем первое слово в динамическую память

  printf("\nEnter second word:\n");
  char* replace = readstr(); // считываем второе слово в динамическую память
  printf("\n");

  for (int w = 0; w < words_count; w++) { // проходим по всем словам
    printf("%s ", words[w]);
    if (strcmp(words[w], word)) { // проверяем на соответствие нужной подстроке
      printf("%s ", replace);
    }
  }
}