...
HugeInt::HugeInt(long long n) : HugeInt(to_string(n)) {}
...
string HugeInt::to_str() {
  string temp;
  if (minus) temp = "-";
  for (int i = digit_count - 1; i > -1; i--) temp += (char)array[i] + '0';
  return temp;
}
...
ostream &operator<<(ostream &out, const HugeInt &a) {
  if (a.minus) out << "-";
  for (int i = a.digit_count - 1; i > -1; i--) out << (char)(a.array[i] + '0');
  return out;
}
...
bool operator!=(const HugeInt &a, const HugeInt &b) { return !(a == b); }
...
bool operator>(const HugeInt &a, const HugeInt &b) { return b < a; }
...
bool HugeInt::is_zero() { return (*this == (HugeInt)0); }

bool HugeInt::has_minus() { return minus; }

// смена знака

const HugeInt HugeInt::operator-() { return HugeInt(*this, true); }
...
HugeInt operator+(HugeInt a, const HugeInt &b) {
  ...
  if (a.has_minus()) {
    if (right.has_minus())
      ...
      return right - (-a);
  } else if (right.has_minus())
    return a - (-right);

  int carry = 0;  // флаг переноса
  for (int i = 0; i < max(a.digit_count, right.digit_count) || carry != 0;
       ++i) {
