...
file *read_file(const char *path) {
  ...
  for (size_t i = 0; i < fsize; i++) str[i] = fgetc(f);
  fclose(f);
...
char lower(int ch) { return ch > 64 && ch < 91 ? ch + 32 : ch; }
...
int is_char(int ch) {
  return (int)((97 <= ch && ch <= 122) || (128 <= ch && ch <= 175) ||
               (224 <= ch && ch <= 239) || (65 <= ch && ch <= 90));
}
...
int compare_words(word *w1, word *w2, int order) {
  ...
  if (w1->size == w2->size)
    ...
    return order ^ (w1->size > w2->size) ? -1 : 1;
}
  ...
  switch (sort_type) {
    ...
    case LENGTH:
      if (w1->size == w2->size)
        ...
        return (reverse ^ (w1->size > w2->size)) ? -1 : 1;
    case VOWELS:
      if (w1->vowels == w2->vowels)
        return 0;
      else
        return (reverse ^ (w1->vowels > w2->vowels)) ? -1 : 1;
    case CONSNTS:
      if (w1->consnts == w2->consnts)
        return 0;
      else
        return (reverse ^ (w1->consnts > w2->consnts)) ? -1 : 1;
    default:
  ...
  for (int i = 0; i < list->size; i++) {
    for (int j = i + 1; j < list->size; j++) {
      ...
      switch (sort_1) {  // оптимизировать
        ...
        case VOWELS:
        case CONSNTS:
          if (!swap) swap = need_swap(node1->word, node2->word, sort_2, order);
          break;
        default:
          printf("Invalid sorting configuration!");
      }
...
//     while ((index = find_word(list, curr -> word -> arr, i + 1, NORMAL)) !=
//     -1)
